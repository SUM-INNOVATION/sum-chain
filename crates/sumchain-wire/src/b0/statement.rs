...
use crate::b0::consts;
use crate::b0::enums::{ObjectKind, UnitKind};
use crate::b0::hashing;
use crate::b0::object_commitment::ObjectCommitmentV1;
use crate::b0::tags::{RESEARCH_CHAIN_TAG, STATEMENT_TAG, STMT_TEMPLATE_PREFIX};
...
impl R0ComputationStatementV2 {
    ...
    /// Re-check exactly what the decoder enforces on constructible state: each of
    /// the eight embedded commitments carries the required object kind for its
    /// slot and is itself internally consistent (`ObjectCommitmentV1::validate`).
    /// The frozen tags, schema version, and fixed scalars are written as constants
    /// by the encoder — never read from a field — so they cannot be made wrong;
    /// the §9/§20 dimension equations remain a separate official-workload layer,
    /// exactly as for the decoder.
    pub fn validate(&self) -> Result<(), DecodeError> {
        let check = |c: &ObjectCommitmentV1,
                     kind: ObjectKind,
                     ctx: &'static str|
         -> Result<(), DecodeError> {
            if c.object_kind() != kind {
        ...
        check(
            &self.updated_token_seq_commitment,
            ObjectKind::TokenSeq,
    ...
    /// `validate()` then encode: the only public route to canonical bytes, so no
    /// public infallible method can emit a statement the decoder would reject.
    pub fn try_encode(&self) -> Result<Vec<u8>, DecodeError> {
        self.validate()?;
        Ok(self.encode())
    }

    /// The fallible `computation_statement_hash` path: `BLAKE3` over the validated
    /// 996 canonical bytes. Never hashes a statement the decoder would reject.
    pub fn try_identity(&self) -> Result<[u8; 32], DecodeError> {
        Ok(hashing::plain(&self.try_encode()?))
    }
...
///
/// Fallible: it validates before emitting bytes, so no template can be produced
/// from a statement the decoder would reject.
pub fn template_bytes(mut statement: R0ComputationStatementV2) -> Result<Vec<u8>, DecodeError> {
    statement.b0_pre_spec_hash = [0u8; 32];
    statement.try_encode()
}
...
mod tests {
    ...
    fn oc(kind: ObjectKind) -> ObjectCommitmentV1 {
        ObjectCommitmentV1::commit(kind, b"payload").unwrap()
    }
    ...
    fn template_has_zero_spec_hash_field() {
        let t = template_bytes(sample()).unwrap();
        assert_eq!(t.len(), 996);
    ...
    #[test]
    fn wrong_kind_embedded_commitment_has_no_infallible_encode_or_hash() {
        let mut s = sample();
        s.model_commitment = oc(ObjectKind::TokenSeq); // valid commitment, wrong kind
                                                       // every public canonical route is fallible and rejects it
        assert!(matches!(
            s.try_encode(),
            Err(DecodeError::Inconsistent {
                ctx: "statement.model_commitment"
    ...
    #[test]
    fn embedded_lying_chunk_count_rejected_at_statement_decode() {
        // Corrupt the model_commitment's chunk_count inside valid statement bytes;
        // statement decode re-derives the expected count and rejects the lie.
        let s = sample();
        let bytes0 = s.encode();
        let mc = s.model_commitment.encode();
        let pos = bytes0
            .windows(mc.len())
            .position(|w| w == mc.as_slice())
            .expect("model_commitment present in statement bytes");
        let mut bytes = bytes0;
        // chunk_count field sits at offset +44 within the 80-byte commitment.
        bytes[pos + 44..pos + 48].copy_from_slice(&99u32.to_le_bytes());
        assert!(matches!(
            R0ComputationStatementV2::decode_exact(&bytes),
            Err(DecodeError::Inconsistent {
