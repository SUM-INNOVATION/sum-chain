...
use crate::b0::consts;
use crate::b0::enums::{Candidate, VerifierMaterialRole};
use crate::b0::tags::VERIFIER_MATERIAL_TAG;
...
impl VerifierMaterialManifestV1 {
    ...
    /// Validate every frozen invariant the decoder enforces: the entry-count cap
    /// (which proves the count fits its `u32` wire field), and per entry a
    /// non-empty printable-ASCII label within `MAX_LABEL_LEN` (which proves the
    /// label length fits its `u16` field), with entries strictly ascending by
    /// `(role, label)` and unique. A struct from `decode_exact` is valid by
    ...
    pub fn validate(&self) -> Result<(), DecodeError> {
        ...
        for e in &self.entries {
            ...
            if label.len() as u64 > MAX_LABEL_LEN as u64 {
    ...
    /// `BLAKE3(canonical_encode(self))`, with no second prefix. Fallible so it
    /// never hashes a structure the decoder would reject.
    pub fn try_identity(&self) -> Result<[u8; 32], DecodeError> {
        Ok(crate::b0::hashing::plain(&self.try_encode()?))
    }

    /// The normative name for the same value, as bound by consumers.
    pub fn try_verifier_material_manifest_hash(&self) -> Result<[u8; 32], DecodeError> {
        self.try_identity()
    }
...
mod tests {
    ...
    fn identity_is_stable_and_sensitive() {
        ...
        let id = m.try_identity().unwrap();
        assert_eq!(id, crate::b0::hashing::plain(&m.encode()));
        let mut m2 = m.clone();
        m2.entries[0].byte_len = 293;
        assert_ne!(m2.try_identity().unwrap(), id);
    }
    ...
    fn identity_is_single_self_domain_not_double_prefixed() {
        ...
        assert_eq!(m.try_identity().unwrap(), crate::b0::hashing::plain(&enc));
        assert_eq!(
            m.try_verifier_material_manifest_hash().unwrap(),
            m.try_identity().unwrap()
        );
        // and explicitly NOT the superseded double-prefixed form
        assert_ne!(
            m.try_identity().unwrap(),
            crate::b0::hashing::prefixed(&VERIFIER_MATERIAL_TAG, &enc)
        );
    }
    ...
    fn identity_changes_with_every_selection_relevant_field() {
        ...
        let id = base.try_identity().unwrap();

        let mut a = base.clone();
        a.candidate = Candidate::Risc0;
        assert_ne!(a.try_identity().unwrap(), id, "candidate_id");

        let mut b = base.clone();
        b.entries[0].role = VerifierMaterialRole::ControlRoot;
        assert_ne!(b.try_identity().unwrap(), id, "role");

        let mut c = base.clone();
        c.entries[0].label = "OTHER_LABEL".into();
        assert_ne!(c.try_identity().unwrap(), id, "label");

        let mut d = base.clone();
        d.entries[0].byte_len += 1;
        assert_ne!(d.try_identity().unwrap(), id, "byte_len");

        let mut f = base.clone();
        f.entries[0].hash[0] ^= 0x01;
        assert_ne!(f.try_identity().unwrap(), id, "digest");
    }

    // ---- Design B: the construction/encoding path also fails closed ----

    #[test]
    fn try_encode_rejects_unsorted() {
        let m = VerifierMaterialManifestV1 {
            candidate: Candidate::Risc0,
            entries: vec![
                e("control_root", VerifierMaterialRole::ControlRoot, 20),
    ...
    #[test]
    fn try_encode_rejects_duplicate() {
        let m = VerifierMaterialManifestV1 {
            candidate: Candidate::Sp1,
            entries: vec![
                e("GROTH16_VK_BYTES", VerifierMaterialRole::Groth16Vk, 1),
    ...
    #[test]
    fn try_encode_rejects_non_printable_label() {
        let m = VerifierMaterialManifestV1 {
            candidate: Candidate::Sp1,
            entries: vec![VerifierMaterialEntry {
                label: "bad\u{7}label".into(), // contains a control byte
    ...
    #[test]
    fn try_encode_rejects_oversized_label() {
        let long = "a".repeat(MAX_LABEL_LEN as usize + 1);
        let m = VerifierMaterialManifestV1 {
            candidate: Candidate::Sp1,
            entries: vec![e(&long, VerifierMaterialRole::Groth16Vk, 1)],
        };
        assert!(matches!(
            m.try_encode(),
    ...
    fn try_encode_rejects_over_cap() {
        let entries: Vec<_> = (0..(MAX_ENTRIES + 1))
            .map(|i| e(&format!("l{i:04}"), VerifierMaterialRole::Groth16Vk, 1))
            .collect();
        let m = VerifierMaterialManifestV1 {
            candidate: Candidate::Sp1,
            entries,
