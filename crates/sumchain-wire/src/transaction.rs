...
use crate::compute_pool_wire::ComputePoolOperation;
...
pub enum TxType {
    ...
    Supply = 26,
    /// C1/ComputePool operations (#130). Append-only, variant index frozen at
    /// **27** (fills the slot the codebase reserved). Carries a
    /// [`crate::compute_pool_wire::ComputePoolOperation`] as opaque `op_bytes`
    /// ([`ComputePoolTxData`]). The wire layout is byte-fixed; EXECUTION is
    /// gate-closed behind `compute_pool_enabled_from_height` (default `None`,
    /// fail-closed) — decoding a ComputePool tx never executes while dormant.
    ComputePool = 27,
    /// Beacon **epoch-setup** operations (BR1 randomness beacon, #125). Append-only,
    /// variant index frozen at **28** — immediately after C1/ComputePool at 27
    /// (#130, registered). Carries a
...
/// C1/ComputePool transaction payload (#130), at the frozen ordinal-27 slot the
/// codebase reserved. Holds the **frozen** canonical
/// [`crate::compute_pool_wire::ComputePoolOperation`] encoding as opaque
/// `op_bytes` — mirrors [`BeaconTxData`]. The consensus bytes are the manual-codec
/// bytes, never a serde re-derivation. Execution/admission decode it via
/// [`ComputePoolOperation::decode_exact`] (7-byte-magic op routing); EXECUTION is
/// gate-closed behind `compute_pool_enabled_from_height` (default `None`), so a
/// ComputePool tx decodes but never executes while dormant — exactly like the
/// beacon carriers at 28/29. Replacing the former uninhabited placeholder in place
/// at position 27 does not shift any beacon bytes.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ComputePoolTxData {
    /// Canonical `ComputePoolOperation` encoding (frozen manual-codec bytes).
    /// Opaque at this layer; routed by its leading 7-byte magic.
    pub op_bytes: Vec<u8>,
}

impl ComputePoolTxData {
    /// Wrap a ComputePool operation as its canonical frozen encoding. `Err` iff
    /// the operation fails its structural encode invariants.
    pub fn from_operation(op: &ComputePoolOperation) -> Result<Self, DecodeError> {
        Ok(Self {
            op_bytes: op.try_encode()?,
        })
    }

    /// Decode the carried bytes back into a `ComputePoolOperation` (magic/op-tag
    /// routing, rejects trailing). Returns a decode error for malformed bytes.
    pub fn decode_operation(&self) -> Result<ComputePoolOperation, DecodeError> {
        ComputePoolOperation::decode_exact(&self.op_bytes)
    }
}
...
pub enum TxPayload {
    ...
    /// **C1/ComputePool** payload (#130, top-level ordinal **27**). Declaration
    /// ordinal 27 == bincode enum tag 27 == `TxType::ComputePool = 27`, preserving
    /// the 1:1 `TxType` ↔ `TxPayload` positional correspondence. Holds
    /// [`ComputePoolTxData`] (opaque canonical `op_bytes`). This replaced the
    /// former uninhabited reserved placeholder **in place** at position 27, so the
    /// beacon variants below keep positional tags 28/29 == their `TxType`
    /// discriminants — no beacon bytes shift. EXECUTION is gate-closed
    /// (`compute_pool_enabled_from_height` = `None`): a tag-27 tx decodes but is
    /// refused at execution while dormant, exactly like the beacon carriers.
    /// **Do not reorder.**
    ComputePool(ComputePoolTxData),
    /// Beacon **epoch-setup** operation (BR1 #125). `TxType` discriminant **28**;
    /// bincode-serialized as the **28th** `TxPayload` variant (**declaration
    /// ordinal 28** — equal to its `TxType`, thanks to the ComputePool slot 27 above).
...
impl TransactionV2 {
    ...
    pub fn tx_type(&self) -> TxType {
        match &self.payload {
            ...
            TxPayload::ComputePool(_) => TxType::ComputePool,
    ...
    pub fn recipient(&self) -> Option<Address> {
        match &self.payload {
            ...
            TxPayload::ComputePool(_) => None, // work-item target/winner resolved at execution, not a transfer recipient
    ...
    pub fn amount(&self) -> Balance {
        match &self.payload {
            ...
            TxPayload::ComputePool(_) => 0, // no token value; pool economics move via executor/state
