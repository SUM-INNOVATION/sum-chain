...
fn engine_identity_is_the_migration_target() {
    // The migration moved the engine identity from the 4.x line to the 7.x line.
    // If this ever silently reverts, mixed-version refusal would compare against
    // the wrong baseline.
    assert_eq!(ENGINE_IDENTITY, "wasmer-singlepass-7");
}
