...
mod tests {
    ...
    fn tiny_model() -> Model {
        ...
        Model {
            ...
            lmhead: {
                let mut m = [[0i16; 16]; 8];
                // make token 3 the argmax via input-independent bias-free head:
                for row in m.iter_mut() {
                    row[3] = 300;
                    row[7] = 100;
                }
    ...
    #[test]
    fn layer_group_runs_and_extends_kv() {
        let t = exp::table_cached();
        let m = tiny_model();
        let x = [100i16, -50, 25, 10, -5, 0, 200, -128];
        // position 0: empty prior kv
        let (out0, kv0) = layer_group(t, &m, &x, &[]).unwrap();
        // position 1: prior kv has one entry
        let (_out1, _kv1) = layer_group(t, &m, &out0, &[kv0]).unwrap();
    ...
    fn select_token_argmax_tie_lowest_and_eos() {
        let m = tiny_model();
        // lmhead makes token 3 win for any nonzero residual
        let (sel, eos) = select_token(&m, &[100; 8]);
        assert_eq!(sel, 3);
        assert_eq!(eos, 0);
        // a model whose head favours token 15 sets eos
        let mut m2 = m.clone();
        for i in 0..8 {
            m2.lmhead[i] = [0; 16];
            m2.lmhead[i][15] = 300;
        ...
        assert_eq!(sel2, 15);
        assert_eq!(eos2, 1);
        // all-zero logits -> tie -> lowest token 0
        let mut m3 = m.clone();
        for i in 0..8 {
            m3.lmhead[i] = [0; 16];
        }
    ...
    #[test]
    fn single_kv_attention_is_value_identity_both_heads() {
        // one KV entry -> weight 65536, den 65536 -> attn_d == v_d for every dim
        let t = exp::table_cached();
        let q = [1000i16, 0, 0, 0, 0, 0, 0, 0];
        let kv = vec![(
            [1000i16, 0, 0, 0, 0, 0, 0, 0],
            [50i16, 60, 70, 80, 11, 22, 33, 44],
        )];
        assert_eq!(attention(t, &q, &kv), [50, 60, 70, 80, 11, 22, 33, 44]);
    }

    #[test]
    fn attention_ignores_entries_beyond_exp_cutoff() {
        // a KV entry whose logit is far below the max gets z > 3016 -> weight 0,
        // so it contributes nothing (result equals dropping it).
        let t = exp::table_cached();
        let q = [32767i16, 0, 0, 0, 0, 0, 0, 0];
        let kv = vec![
            (
                [32767i16, 0, 0, 0, 0, 0, 0, 0],
                [100i16, 0, 0, 0, 0, 0, 0, 0],
            ),
            (
                [-32768i16, 0, 0, 0, 0, 0, 0, 0],
                [-100i16, 0, 0, 0, 0, 0, 0, 0],
            ),
        ];
        assert_eq!(attention(t, &q, &kv)[0], attention(t, &q, &kv[..1])[0]);
    }

    #[test]
    fn one_step_attention_score_diverges_from_two_step() {
        use crate::fixed::{rhaz, saturate};
        // frozen one-step score: saturate(RHAZ(sum q·k, 512))
        assert_eq!(saturate(rhaz(100 * 100, 512)), 20);
        // a naive two-step (pre-divide q,k then combine) gives a different score
        let two_step = rhaz(rhaz(100, 16) * rhaz(100, 16), 2);
        assert_ne!(two_step, 20);
    }
    ...
    fn extreme_inputs_do_not_overflow() {
        let t = exp::table_cached();
        let m = tiny_model();
        layer_group(t, &m, &[i16::MAX; 8], &[]).unwrap();
        layer_group(t, &m, &[i16::MIN; 8], &[]).unwrap();
        // negative values flow through RMSNorm/attention without panic
        layer_group(t, &m, &[-1, -2, -3, -4, -5, -6, -7, -8], &[]).unwrap();
    }
}
