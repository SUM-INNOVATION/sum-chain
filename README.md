...
| OmniNode `InferenceAttestation` | In development (`omninode-attestation` branch) — Phases 1–3 complete (wire format + parity gate + executor dispatch + storage CF + activation param + mempool admission with permanent CF dedup); Phase 4 (production wiring + read-only RPC) ahead | `crates/primitives/src/inference_attestation.rs`, `crates/state/src/inference_attestation_executor.rs`, `crates/state/src/mempool.rs` (`InferenceAttestationAdmission`), fixtures in `crates/primitives/tests/fixtures/` | Verifier-signed digests attesting to off-chain inference outputs. Inner Stage 6 signature (`omninode.inference_attestation.v1` domain) verified at chain side; outer chain signing semantics unchanged. Activation gated by `omninode_enabled_from_height` (default `None` — dormant on mainnet). Mempool admission enforces activation gate + in-flight duplicate + permanent CF duplicate so zero-fee executor failures are unreachable in production. |
...
- **Phase 3 (mempool admission):** shipped on `omninode-attestation` branch. New `InferenceAttestationAdmission` carries the narrowest context (executor, ChainParams, current-height) into the mempool via `Mempool::with_inference_admission(...)`. `Mempool::add` performs three checks before insert: (a) activation gate (`StateError::OmniNodeNotActivated`), (b) in-flight `(session_id, verifier_address)` duplicate (`StateError::DuplicateInferenceAttestation`), (c) permanent `INFERENCE_ATTESTATIONS` CF duplicate (same error). In-flight key is stamped after every other index insert succeeds and cleared on `remove`/`clear`. Non-`InferenceAttestation` payloads are unaffected. 11 admission tests + shared `tests/common/mod.rs` helpers reused across dispatch and admission suites.
- **Phase 4 (production wiring + read-only RPC)** — ahead. (a) Wire `InferenceAttestationAdmission` into node startup so `sum_sendRawTransaction` enters a mempool with admission enabled (without this, the Phase 3 admission hook is dormant in production); (b) thread live block-height into `current_height: Arc<AtomicU64>`; (c) integration assertion that the production submit path is admission-gated; (d) implement `sum_getInferenceAttestation`, `sum_listInferenceAttestations`, `sum_getInferenceAttestationStatus`.

**Last Updated**: 2026-05-13 (Phase 3 mempool admission complete)
