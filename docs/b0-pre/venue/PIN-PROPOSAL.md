...
| 8 | `prover_archives` — per (archive, arch): `archive_url` + `archive_sha256`, and per **member**: `executable_name` + `member_path` + `member_sha256` + `member_size_bytes` + `version_argv` + `version_output` + `expected_release_commit` + `delivery` | the pinned SP1 / RISC Zero prover release **archives** and the exact executables inside them | `verify_pins.sh` validates the SHAPE + COVERAGE + shared-archive + delivery + no-`rzup` invariants (each `archive_sha256` / `member_sha256` a bare 64-hex; `member_size_bytes` a positive integer; `version_output` carries `expected_release_commit`; `delivery` is `isolated-path` for cargo subcommands and `risc0-server-path` for `r0vm`; `cargo-prove` present for **x86_64 AND aarch64**, `cargo-risczero` + `r0vm` **x86_64-only** and **sharing one archive**; a swapped-arch / wrong-member / wrong-size / altered-digest / incorrect-version / forbidden-`r0vm` value all fail closed). It THEN **independently downloads each archive** and re-verifies it through the single verified-extraction implementation (`provision_prover_toolchain.sh`): whole-archive `sha256`, safe **complete** enumeration, complete member set, and each member's `member_size_bytes` + `member_sha256` against the **actual bytes**; and cross-binds each `archive_sha256` to the primary-source-verified `tool_identity.checksum_hex` for the **same url**. So `member_sha256` / `archive_sha256` — which have **no other independently verified duplicate** — are authority-verified pre-flight, not merely shape-checked and then trusted to the in-image provisioner. The same values remain content-verified **in-image** at build time by that same staged provisioner (below) and golden-tested against the transferred content-addressed venue report |
