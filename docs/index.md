...
| `omninode_sponsored_attestation_enabled_from_height` | `null` | Sponsored inference attestation (v2 envelope) **implemented, not yet gated** (code-backed, issue #79; a payer/sponsor may submit an attestation, but the attestation stays verifier-keyed for dedup/storage/settlement). Not part of the 8,900,000 cohort — set a height to activate. |
