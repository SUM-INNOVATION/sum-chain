...
So the verdict stays REACHABLE, and the blocking count is untouched by any of
it — 121 at the audit, less OC-2, which the integration pass closed outright
rather than gated. (Later passes took it to 120, then 114, and a COUNT at this
head puts it at **107**; see Counts, where the difference is traced to seven SC
rows the Class 7 pass closed without moving the table. None of those passes
moved a gate or a line of executing code on the rows this paragraph is about,
which is why its claim is untouched by them.)
...
a deployment decision: setting heights in the runtime `genesis.json` of
every validator — twelve when this paragraph was written, thirty-seven now —
as one coordinated consensus-breaking activation. That decision
...
A third status is therefore recorded alongside the verdict, orthogonal to it:
  ...
  * **REMEDIED, PENDING ACTIVATION** — the corrected behaviour exists, is reachable
    ...
    sets it. Most blocking rows fully; AU-34, AU-35, AU-37, RY-2, D-19b and the
    transfer-and-subdivide remainder of OV-22 in part, and the tables say which
    part. AU-3 was in that partial list and no longer is: its GRANT half was
    solved rather than refused.
  * **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION** — the defect cannot be
    closed by a guard, so the operation REFUSES instead: a FAILED receipt naming
    the operation UNSUPPORTED, returned ahead of the fee deduct, at its own
    activation height. Not a repair and not claimed as one; the point is that
    "structurally blocked" may not be taken to mean "left reachable", because an
    operation that cannot be performed correctly must not proceed on a guess.
    The seven `VerifyProof` rows, AU-9, AU-10, AU-11, AU-18, AU-21, AU-32 and
    RY-1.
  * **BLOCKED, STRUCTURAL** — the defect cannot be closed by a guard at all,
    ...
    authorize against, AND refusing would take away a capability rather than an
    attack. A wire change or a new registry is needed. Stated rather than
    papered over, because a rule that pretends to check something it cannot see
    is worse than one that says it cannot. OV-11, and the transfer and subdivide
    halves of OV-22.
...
moved is what this document claims to know.** Blocking fell from 120 to 114 at
that pass; a later COUNT at this head puts it at 107, and both derivations are
in the Counts section.
...
| AU-3 | A consent's subject is never consulted in either direction: `GrantConsent` compares only issuer to sender, `RevokeConsent` requires the issuer, so the subject can neither grant nor withdraw | Healthcare §missing authorization | **REACHABLE** | none today. **REMEDIED, PENDING ACTIVATION**, in BOTH halves and behind TWO heights: `healthcare_authorization_enabled_from_height` (the REVOCATION half, already dormant here) and `healthcare_consent_subject_signature_enabled_from_height` (the GRANT half), both implemented and dormant. The earlier note filed the grant half **BLOCKED, STRUCTURAL** on the reasoning that "a subject cannot participate in granting without a signature `ConsentEnvelope` does not carry". That was wrong about WHAT is missing, and the row is corrected rather than quietly moved: `ConsentEnvelope` DOES carry `subject_address`, so the gap was a SIGNATURE, not an address -- which makes it a payload change and not a registry, and it was SOLVED rather than refused. At and above the grant height the payload is a new wire type `ConsentGrantRequest`: the envelope, the subject's ed25519 public key, and the subject's signature over a domain-separated blake3 digest, `ConsentEnvelope::grant_signing_input` under `SRC874-CONSENT-GRANT:v1:`. The key must derive to the envelope's OWN `subject_address` and the signature must verify, ed25519, with three distinct refusals so a receipt says which condition failed; the issuer must still be the sender, so an accepted `GrantConsent` carries BOTH parties. A WRAPPER rather than two more fields on the envelope, because the envelope is what this subsystem stores, and each encoding fails to decode the other rather than silently reinterpreting it. Its own height rather than the authorization one, because that gate changes which SENDER an arm accepts and this one changes what a payload IS. **The guarantee holds only with the other height ALSO open**, and the source says so: with `authorization` closed `SupersedeConsent` checks nothing about the sender and mints the very record this gate refuses to let `GrantConsent` mint. That is AU-1's arm and AU-1's row, and it is pinned here rather than left for a reader to discover | `crates/state/src/healthcare_executor.rs:596` guard `:600`; `:643` guard `:656`; `subject_address`/`subject_ref` declared at `crates/sumchain-wire/src/healthcare.rs:636,642`; pinned by `the_subject_of_a_consent_can_neither_grant_nor_revoke_it`; grant remedy `HealthcareExecutor::consent_subject_signature_gate_open` (`crates/state/src/healthcare_executor.rs:356`) read in the `GrantConsent` arm at `:1199`, ahead of the arm's own issuer and duplicate refusals and ahead of `v_deduct` at `:1226`; the three checks in `subject_agreed` (`:379-390`); wire type `ConsentGrantRequest` (`crates/sumchain-wire/src/healthcare.rs:853-862`) and signing input `ConsentEnvelope::grant_signing_input` (`:770`) under `CONSENT_GRANT_SIGNING_SEP` (`:47`); mixed-version `a_consent_is_granted_without_the_subject_below_the_gate_and_not_above_it`, `a_consent_signed_by_a_key_that_is_not_the_subjects_is_refused`, `a_subject_signature_does_not_carry_to_a_wider_disclosure`, `the_issuer_must_still_be_the_sender_at_the_gate` and `the_grant_gate_alone_does_not_close_supersession` (`sumchain-state --test consent_subject_signature_gate`) | disclosure authorizations about a person are recorded without their participation -- below both heights, which is what a release-configured node runs. The row is no longer PARTLY remedied, and the reason is worth stating: the half that was called structural was structural about the ENVELOPE and not about the subsystem, and a wire type is a thing a remediation pass may add |
...
| AU-7 | Issuer identity is self-asserted: where a check exists it is `issuer_address == sender`, and `issuer_address` comes from the payload on every creation path; no issuer registry is consulted | Healthcare / Property §missing authorization | **REACHABLE** | none — **DEFERRED, NO UNSAFE OPERATION REACHABLE, PINNED.** Re-examined this pass under "deferred functionality is acceptable; unsafe ambiguity is not" and deliberately NOT gated. The only place an `issuer_class` decides anything is Finance's three `can_issue_*` checks, and those resolve against the registry that `subsystem_issuer_self_registration_unsupported_enabled_from_height` already refuses to let an applicant write; in Healthcare and Property the field is stored and read by no guard. It is also mandatory on every creation, so failing it closed would refuse the entire creation surface of two subsystems for no safety gain. The field decides nothing, so there is no ambiguity for the rule to bite on | Healthcare creation guards e.g. `crates/state/src/healthcare_executor.rs:600, 712`; Property `AnchorAsset` guard `asset.issuer_address != *sender` in `crates/state/src/property_executor.rs`, with no `v_get_issuer` call anywhere in `property_executor.rs` or `property_view.rs`; **pinned by `any_funded_account_is_an_issuer_of_any_class_it_names_for_itself` and `an_issuer_address_that_is_not_the_sender_is_the_one_thing_refused`** | the check binds a row to whoever created it and to nothing else. **Pinned, not remedied, and deliberately so:** an issuer registry is a rule about WHO may write, so adding one changes which transactions succeed and is a consensus change that may not arrive ungated -- and this tree has no Property or Healthcare issuer registry for an executor to consult, so supplying one means writing standard, which is the AU-8 and AU-33 blocker in another subsystem. The pin states the row in the direction a regression would trip: a key that has registered nothing, staked nothing and been admitted by nobody anchors an asset declaring itself a `LandRegistry` and succeeds, the self-asserted class is what gets STORED, and a SECOND unrelated account claims the same class on its own asset and also succeeds -- so there is no admission and nothing to be admitted to. The second test is the discriminator that keeps the first honest: an `issuer_address` naming an address other than the sender IS refused and writes nothing, which is exactly the row's sentence that where a check exists it is `issuer_address == sender` and nothing more |
| AU-8 | `policy_id` carried on providers, memberships, consents, prescriptions (and on Property assets, title events, encumbrances, coverage, claims; and on Agreement commitments, attestations, IP actions, executor links) — stored, consulted by no guard | Healthcare / Property / Agreement | **REACHABLE** | none. **EXAMINED AND LEFT, with the blocker named: making the field gate anything requires deciding WHAT it names, and the one registry it could plausibly resolve against is a different namespace.** `PolicyAccountExecutor::v_get_policy_account` exists and is reachable from the state crate (`crates/state/src/policy_account_view.rs:46`), so "consult the registry" is mechanically available; what is missing is any statement that a membership's `policy_id` is a policy ACCOUNT id rather than a commitment to an off-chain policy document. No subsystem executor names `PolicyAccount` at all, no `policy_id` value in any fixture is a policy-account key, and the two are typed identically (`[u8; 32]`) so the compiler cannot tell them apart either. Binding them would be inventing the binding, and binding them WRONG refuses every lawful transaction whose `policy_id` is a document commitment — the AU-33 failure mode — **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `subsystem_ambiguous_policy_id_refused_enabled_from_height`, implemented and dormant. The namespace question is NOT resolved and this row still says so -- the gate refuses a non-zero `policy_id` rather than binding it, because binding it wrong refuses every lawful transaction whose id is a document commitment. 16 arms and not the 13 named here: the 3 supersession arms write a replacement of the same type, and a creation-only gate would be bypassable through them. `[0u8; 32]` stays accepted as the absent sentinel. Pins `a_property_asset_claims_an_unresolvable_policy_below_the_gate_and_not_above_it` and its two siblings, `every_arm_that_stores_a_policy_id_calls_the_refusal` | in the state crate `policy_id` appears only at `crates/state/src/agreement_executor.rs:684`, inside the `#[cfg(all(test, feature = "legacy_tests"))]` module opening at `:634`; the candidate registry it does NOT resolve against is `crates/state/src/policy_account_view.rs`; **pinned by `a_policy_id_that_resolves_to_nothing_is_accepted_and_changes_no_decision`** | the field exists, is written, and gates nothing — and it cannot be made to gate anything until the tree says which namespace it is in. **The row stays EXAMINED AND LEFT and is now PINNED**, in three parts, the third of which is what makes it a pin on the FIELD rather than on one value of it: a Property `AnchorAsset` whose `policy_id` names no policy account succeeds and the id is STORED; `PolicyAccountExecutor::v_get_policy_account`, called from the test at that same id, reports it absent -- so the accessor is reachable, would have answered, and simply is not called on the execution path; and two anchors differing ONLY in `policy_id` produce rows that differ only in `policy_id`, so nothing downstream of the field reads it. A guard that begins resolving the field fails the first assertion; a field that begins reaching any other stored value fails the third |

### Agreement — no authorization outside attestations

| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| AU-9 | A signature's party reference comes from the payload and is never compared to the sender, so any sender signs on behalf of any party and carries a two-party agreement to `Executed` alone | Agreement §missing authorization | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `agreement_party_authority_unsupported_enabled_from_height`, implemented and dormant. The structural fact is unchanged and is why the remedy is a REFUSAL rather than a guard: `AgreementCommitment` carries no address at all and `PartyRef` is a 32-byte commitment or a 32-byte subject id, so there is nothing to compare a sender to, and an executor that picked a mapping from `PartyRef` to `Address` would be enforcing a rule nobody set. What changes is that "structurally blocked" stops meaning "left reachable": at and above the gate `SignAgreement` -- and `RevokeSignature` beside it, because authorizing the writing of a signature while leaving any sender able to delete one is not an authorization rule -- returns a FAILED receipt carrying `AGREEMENT_PARTY_AUTHORITY_UNSUPPORTED`, before the deduct, where this executor's other refusals already return. The wire change stays the real fix and stays unwritten | arm `crates/state/src/agreement_executor.rs:275`; `party_ref` deserialized at `:294`, used at `:297`; guards are agreement existence `:280` and duplicate signature `:284` only; pinned by `any_sender_can_sign_on_behalf_of_any_party`; remedy `AgreementExecutor::party_authority_unsupported_gate_open` (`crates/state/src/agreement_executor.rs:293`) read at `:484`, the arm set enumerated exhaustively at `:485-509` and the refusal returned at `:512` ahead of every `v_deduct`; mixed-version `a_stranger_signs_and_terminates_below_the_gate_and_neither_above_it` and `every_agreement_arm_that_needs_a_party_refuses_and_only_those` (`sumchain-state --test structural_refusal_gates`) | consent itself is forgeable in one transaction |
| AU-10 | The `signature` bytes are stored and never verified against `signer_key` or anything else | Agreement §missing authorization | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `agreement_party_authority_unsupported_enabled_from_height`, implemented and dormant, the same field AU-9 and AU-11 read. The structural fact is unchanged: verifying the stored `signature` against `signer_key` needs a canonical signing input this subsystem defines nowhere, and that is a wire change. At and above the gate the arm that WRITES the unverifiable signature refuses as UNSUPPORTED before the deduct, so the chain stops accumulating bytes it has no rule to check. One field for all three rows rather than three, because they are one absence seen from three sides | field `crates/sumchain-wire/src/agreement.rs:289`, stored via `v_put_signature` `crates/state/src/agreement_executor.rs:296`; no `verify`/`ed25519` call exists in `agreement_executor.rs` or `agreement_view.rs`; same gate, same two tests as AU-9 | nothing in the executor checks a signature |
| AU-11 | Any sender may terminate, void or supersede any agreement, revoke any IP action, and activate, pause, resume, terminate or complete any executor link | Agreement §missing authorization | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `agreement_party_authority_unsupported_enabled_from_height`, implemented and dormant, the same field AU-9 and AU-10 read. Same missing address as AU-9, so same answer: the fourteen arms that need a party -- the eight this row names, plus `SignAgreement` and `RevokeSignature` from AU-9 and AU-10, plus `UpdateAgreement`, which this row does not name but which writes an `AgreementStatus` straight from the payload and therefore REACHES `Terminated`, `Voided` and `Superseded`, the three states this row is about -- return a FAILED receipt carrying `AGREEMENT_PARTY_AUTHORITY_UNSUPPORTED` before the deduct. The ten that do not are left reachable and the gate says which: `CommitAgreement`, `AddParty`, `RemoveParty`, `RecordIpAction` and `LinkExecutor` create a row under a fresh id and damage nothing that exists; the three attestation arms already check `issuer_address == sender`; `SubmitProof` and `VerifyProof` are PR-2's row. The family keeps a way to record an agreement; what it loses is a stranger's ability to change one | arms and their sole existence/state guards: `crates/state/src/agreement_executor.rs:212`/`:220`, `:242`/`:251`, `:424`/`:432`, `:472`/`:480`, `:502`/`:510`, `:526`/`:534`, `:555`/`:563`, `:579`/`:587`; pinned by `any_sender_can_terminate_void_and_revoke_anything`; same gate, same two tests as AU-9, the second of which asserts the arm set exhaustively rather than by sample | none of the eight compares the sender |
...
| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| AU-18 | Anyone self-registers an ACTIVE issuer with an arbitrary class, `TaxAuthority` included, and then issues claims | Tax 1 | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `subsystem_issuer_self_registration_unsupported_enabled_from_height`, implemented and dormant. The structural fact is unchanged and is why the remedy is a REFUSAL rather than a registrar: no `ChainParams` field names a tax registrar, nothing in `genesis.json` seeds the registry, and no governance path in this tree writes to it, so inventing an authority inside an executor would be a rule nobody set and choosing WHICH classes may self-assert would be the same invention in a smaller font. At and above the gate `RegisterIssuer` returns a FAILED receipt carrying `ISSUER_SELF_REGISTRATION_UNSUPPORTED`, before the deduct, where this arm's own duplicate refusal returns. **The cost is stated rather than hidden:** with no registrar a chain at this height cannot obtain a Tax issuer at all, so every arm that requires one is unsatisfiable and the subsystem is DEFERRED -- and nothing that ever worked is stranded, because there has never been a lawful issuer here to strand | arm `crates/state/src/tax_executor.rs:117`; guards are `issuer.address != *sender` `:121` and "already registered" `:125`; `tax_class` and `status` come from the payload (`crates/sumchain-wire/src/tax.rs:413, 425`), and `TaxIssuerClass::TaxAuthority` is a plain variant at `crates/sumchain-wire/src/tax.rs:314`; remedy `subsystem_issuer_self_registration_unsupported_gate_open` (`crates/state/src/lib.rs:458`) read at `crates/state/src/tax_executor.rs:362`, ahead of the decode and of `v_deduct`; mixed-version `a_tax_authority_registers_itself_below_the_gate_and_not_above_it` (`sumchain-state --test structural_refusal_gates`) | the registry authorizes nothing it does not take from the applicant |
...
| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| AU-21 | Any sender self-registers as any finance issuer class, `CentralBank` included, and attests KYC in the same block | Finance 1 | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `subsystem_issuer_self_registration_unsupported_enabled_from_height`, implemented and dormant, the SAME field AU-18 reads, and one field rather than two on that field's own argument: one rule, two identical bodies, the same blast radius on each side, and no configuration in which an operator wants a registry to authorize registrations in Finance and not in Tax. Same shape as AU-18, for finance issuer classes, and the same DEFERRAL of the subsystem behind it | arm `crates/state/src/finance_executor.rs:148`; guards `:152` (profile names sender) and `:156` (duplicate); `issuer_class` deserialized from the payload at `:149`; remedy the same accessor read at `crates/state/src/finance_executor.rs:373`, ahead of the decode and of `v_deduct`; mixed-version `a_central_bank_registers_itself_below_the_gate_and_not_above_it` (`sumchain-state --test structural_refusal_gates`) | source-only in the blocker document; established here from source |
...
| AL-13 | The claim these measurements cannot support: that arbitrary input never reaches an allocator abort, because the replacement value is built before the ceiling is charged | stated as a scope limit by Tax, Legal, Finance, Agreement, Property, Healthcare, NFT and DocClass | **UNDETERMINED** | — — **OWNER ACTIVATION DECISION, TAKEN.** The missing third number has been supplied by decision rather than found in the tree: 4 GiB is the supported validator memory floor for this release, an OPERATIONAL minimum and explicitly not a consensus-derived fact -- it is in no `ChainParams` field, no activation digest and no `consensus_limits` entry, and nothing compares it at runtime. This row's own objection stands unweakened and is why the decision was needed: `reference_memory_bytes` was NOT promoted, and the b0-pre artifact's refusal to state a hardware minimum is an ELIGIBILITY claim, which the support floor does not contradict. All four validator StatefulSets now request as well as limit 4Gi -- they previously requested 1Gi, so the envelope the 256 MiB ceiling is derived from was never actually reserved. See `docs/operations/validator-memory-floor.md`; pinned by `the_recorded_validator_memory_envelope_is_still_what_the_derivation_assumed` | the eight measurements are each one point; the release ceiling is 1 GiB (`crates/state/src/executor.rs:269`), not the 4,096/8,192 B the measurements used | **what would settle it:** a growth-to-abort analysis at the release ceiling — the maximum row size reachable under `max_block_bytes: 2000000` and a 1 GiB write-set ceiling, multiplied by the 2× (index) and 3× (in-row) allocation factors these measurements establish, compared against the memory a validator is specified to have. **Two of those three numbers are now measured** (see the preamble): the growth rate is 1,899,873 B per transaction against a DocClass identity row, and the allocation factor is 4.00x peak / 5.00x cumulative for the row-field cases, linear over six doublings. The third is still absent — this tree states no memory a validator is required to have — so the row stays UNDETERMINED, but it is now undetermined for ONE missing number rather than three. The gate below removes the growth rate from the question for AL-9, AL-10 and AL-11; it does not remove it for the rows that are still `none`. **Re-checked for this pass, and the third number is STILL ABSENT**: `docs/`, `README.md`, `RELEASE.md`, `configs/` and `deploy/` state no minimum memory a validator is required to have, so the row does not move. What HAS changed is the size of the remainder: with AL-1 through AL-12 now all reading `subsystem_allocation_bound_enabled_from_height`, the growth rate is removed from the question for EVERY row in this class rather than for three of them, and no row in it carries an unbounded growth rate at activation. The row stays UNDETERMINED because the question it asks is about the CEILING and the hardware, not about the accumulators: bounding every accumulator does not tell you whether a 1 GiB write set multiplied by a 4.00x peak factor fits in the memory a validator has, when nothing states what that is. **Re-checked again in the findings pass, and the third number has NOT arrived — but the search now returns something, and what it returns sharpens the row rather than settling it.** `docs/b0-pre/protocol/b0-pre-protocol-v1.json` carries `reference_memory_bytes: 4294967296` (4 GiB) under `qualification_gates`, attached to validators, and it is the only validator-attached memory figure in this tree. **It is not the missing number and its own text forbids reading it as one**: the same object says the figure is "only the controlled candidate-comparison envelope, not a deployment or consensus hardware minimum", its `scope` repeats "This is a candidate-comparison envelope, not a validator hardware minimum", and its `validator_eligibility` states outright that qualification is "performance-based, not hardware-class-based: no minimum physical cores or RAM, and detected host hardware is never an eligibility gate. A node of any CPU/RAM configuration may participate." `docs/b0-pre/venue/VENUE.md:297` and the schema at `:920` say the same. So the position is stronger than "the number is absent": **the tree affirmatively states that no validator RAM minimum exists**, which means the comparison this row asks for has no right-hand side to be supplied from the tree's own statements, and inventing one — by promoting the 4 GiB envelope, which is the one figure a careless pass would reach for — would be manufacturing the very specification the document refuses to make. **This row therefore CANNOT be closed from source and configuration, in this pass or any other that stays inside the tree.** Settling it is a specification decision: either the project states a validator memory floor, at which point the growth-to-abort analysis becomes arithmetic over three known numbers, or the eight subsystems stop making the claim their measurements cannot support. Both are outside a remediation track's authority. It is recorded here as blocking, with the reason, rather than left to be re-searched each pass. **THE SPECIFICATION DECISION NAMED IMMEDIATELY ABOVE HAS NOW BEEN TAKEN, and it is the first of the two branches:** the repository owner has adopted **4 GiB (4,294,967,296 B) as the SUPPORTED VALIDATOR MEMORY FLOOR for this release**, stated in `docs/operations/validator-memory-floor.md`, echoed at `docs/b0-pre/venue/VENUE.md`, and reserved by every shipped validator manifest in `deploy/kubernetes/` as `requests.memory: "4Gi"` alongside the `limits.memory: "4Gi"` that was already there (the request was `1Gi`, which is Burstable QoS and evictable far below the floor, so the envelope was not in fact reserved). **This does NOT promote `reference_memory_bytes` into the missing number, and the objection above stands unweakened**: the b0-pre figure remains the controlled candidate-comparison envelope and is not a deployment or consensus minimum, its text is untouched, and its ratified `b0_pre_spec_hash` is unchanged. The floor is a separate, OPERATIONAL statement — not consensus, not an eligibility gate, detected by nothing, folded into no digest — which is precisely the kind of statement this row said would have to be made, and which no amount of reading could manufacture. **What this changes for the row:** the right-hand side of the comparison now exists, so the row is no longer blocked on a number that is absent. **What it does not change:** the row STAYS **UNDETERMINED**, because the growth-to-abort analysis over the three numbers has not been performed here and this track is not performing it. The blocker has moved from "one of the three inputs does not exist anywhere" to "the arithmetic over three existing inputs has not been done" — which is an INTERNAL blocker, settleable in this worktree, and the first time this row has been in that bucket. The 4 GiB figure is also what `MAX_BLOCK_WRITE_SET_BYTES` (256 MiB) has always been derived against; that derivation is now coherent rather than resting on an envelope the tree denied |
...
| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| RY-1 | `royalty_bps` and `royalty_recipient` are stored, returned by the RPC readers, and consulted by no execution path; a transfer pays the recipient nothing | NFT §royalties and config | **REACHABLE** | none today. **REMEDIED AS UNSUPPORTED, PENDING ACTIVATION:** `nft_unpayable_royalty_refused_enabled_from_height`, implemented and dormant. Everything the three-level finding below establishes is unchanged, and it is exactly why the remedy is a REFUSAL rather than a payment: no transfer on this chain carries consideration for a royalty to be a fraction of, a price field alone would authorise debiting an account whose holder signed nothing, and paying one is a protocol -- a two-sided order, a standing listing or an escrowed bid -- and not something an executor may decide. What changes is that the chain stops RECORDING a promise it has no code to keep: at and above the gate a `CreateCollection` whose `royalty_bps` is non-zero returns a FAILED receipt carrying `UNPAYABLE_ROYALTY_UNSUPPORTED`; a collection with no royalty is created exactly as before. Refused after `validate()`, so a malformed config is still reported as malformed, and before the id is computed, so a refused creation consumes no collection id. **Creation is the only place this can be enforced, and that is a fact about the wire rather than a choice:** `NftUpdateCollectionConfigData` carries no `new_royalty_bps` at all (RY-2's second half), so `royalty_bps` can never be changed after creation. Collections created BELOW the height keep what they recorded -- a gate changes what a node does next, not what a chain has already written | written `crates/state/src/nft_executor.rs:272-277` and `:723`; schema `crates/storage/src/schema.rs:938, 940`; read only for display at `crates/rpc/src/server.rs:1998-1999`, `:8142-8143` and `crates/wallet/src/main.rs:1521-1522`; `execute_transfer` (`crates/state/src/nft_executor.rs:483-542`) and `v_transfer_token` (`crates/state/src/nft_view.rs:279-297`) move no balance at all; pinned by `royalties_are_recorded_and_never_paid`; remedy `NftExecutor::unpayable_royalty_refused_gate_open` (`crates/state/src/nft_executor.rs:441`) threaded at `:684` and read at `:858`; mixed-version `a_collection_records_an_unpayable_royalty_below_the_gate_and_not_above_it` (`sumchain-state --test structural_refusal_gates`) | the field is publicly readable over RPC (`nft_getCollection`, `crates/rpc/src/api.rs:604`), so a marketplace is told a royalty exists that the chain will never pay |
...
| OV-21 | Property: only `ReinstateCoverage`, `PayClaim` and `ReopenClaim` guard on the state they read; every other transition applies from any prior status | Property §invalid transition | **REACHABLE** | none today. **REMEDIED, PENDING ACTIVATION:** `property_state_precondition_enabled_from_height`, implemented and dormant. At and above the gate a row whose status is FINAL accepts no further operation, where FINAL means the status a NAMED operation writes and that no named operation leaves -- read off the arms rather than chosen here: `Merged`, `Subdivided` and `Deregistered` for an asset, and the terminal statuses of the title-event, encumbrance, coverage and claim families beside them. Twenty-two guard sites, one per guarded arm across the five families, and every one of them ahead of that arm's `v_deduct`, so a refused transition writes nothing and costs nothing -- which is where this executor's own pre-existing refusals already return. Deliberately NOT a general transition table: which non-final status may follow which is a policy this tree does not state, and inventing one would refuse lawful transitions to close a hole that is about the statuses nothing lawfully leaves | arms in `crates/state/src/property_executor.rs`; guarded exceptions only; remedy `PropertyExecutor::state_precondition_gate_open` (`crates/state/src/property_executor.rs:325`), read at twenty-two `gates.state_precondition` sites (`:754, 789, 842, 846, 921, 956, 1030, 1075, 1128, 1203, 1239, 1276, 1313, 1391, 1422, 1453, 1490, 1598, 1630, 1666, 1733, 1803`), each four to twenty-three lines ahead of its own arm's `StateManager::v_deduct`; finality read off the arms by `asset_status_is_final` (`:368`) and its four siblings; mixed-version `the_final_asset_statuses_are_the_ones_a_named_operation_writes_and_none_leaves` and its four siblings, `every_guarded_asset_arm_refuses_a_final_asset` and its four siblings, `a_paid_claim_is_closed_reopened_approved_and_paid_again_only_below_the_gate`, and `below_the_gate_every_asset_status_accepts_the_free_form_update` and its four siblings (`sumchain-state --test property_decided_state_gates`) | a `Deregistered` asset returns to `Active`; a `Paid` claim moves anywhere -- below the gate, which is still what a release-configured node runs |
| OV-22 | Property `MergeAssets` records no relationship and never writes the primary asset; `SubdivideAsset` creates no children; `TransferAsset` moves no ownership — an asset row has no owner field | Property §invalid transition | **REACHABLE** | none today. **REMEDIED, PENDING ACTIVATION** on the MERGE half: `property_asset_relationship_enabled_from_height`, implemented and dormant. At and above the gate `MergeAssets` records the relationship on BOTH rows -- each names the other in `related_assets`, the field `AssetAnchor` carries "for subdivisions, mergers" -- idempotently (`contains` then push, so merging the same pair twice adds no second entry), and after the status write, so a reader that sees `Merged` never sees it without the link in the same candidate. The accumulating list is BOUNDED in the same arm: both stored rows are length-probed against `MAX_ACCUMULATING_ROW_BYTES` and the merge is refused before the fee when either exceeds it, which is AL-6's rule travelling with the write rather than behind the allocation height, because this gate is what turns two fixed-size records into accumulating ones. The TRANSFER and the SUBDIVIDE halves stay **BLOCKED, STRUCTURAL** and are untouched: an asset row has no owner field for `TransferAsset` to move and no child ids for `SubdivideAsset` to create, and both are wire changes to `crates/sumchain-wire`, not executor changes | `crates/state/src/property_executor.rs:805` writes `AssetStatus::Merged` onto the secondary at `:867-872`; `:902` writes only `Subdivided` at `:928-933`; `:770` writes only `PendingTransfer` at `:796-801`; pinned by `merge_subdivide_and_transfer_record_a_status_and_nothing_else`; merge remedy `PropertyExecutor::asset_relationship_gate_open` (`:350`) read at `:884`, the two `v_add_related_asset` calls at `:885` and `:891` (`crates/state/src/property_view.rs:200`, the committed twin of DE-7's uncalled `PropertyAssetStore::add_related_asset`, statement for statement), the bound probe at `:857-862` through `asset_row_within_bound` (`:437`); mixed-version `a_merge_links_both_rows_only_above_the_relationship_gate`, `merging_the_same_pair_twice_adds_no_second_entry`, `a_merge_is_refused_when_either_stored_asset_row_is_already_over_the_bound` and `each_new_height_opens_its_own_gate_and_neither_opens_the_other` (`sumchain-state --test property_decided_state_gates`) | three operations that name an effect and produce a status. One of the three now produces its effect above a height nobody has set; the other two still cannot, and the row says which is which rather than moving whole |
...
| DE-7 | `AssetStore::add_related_asset` — the one writer that could record a merge or subdivision | Property §invalid transition | **UNREACHABLE** | defined `crates/storage/src/property_store.rs:316`; still no caller repo-wide — the merge remedy reproduces it against the block's candidate as `PropertyAssetView::v_add_related_asset` (`crates/state/src/property_view.rs:200`) rather than calling the committed twin, because a committed write inside execution is what this tree forbids by construction | this was *why* OV-22 was true of all three halves. It stays true of the SUBDIVIDE half, which records no relationship at any height; the merge half is remedied above `property_asset_relationship_enabled_from_height` through the view twin, and this row's own verdict is unaffected because nothing calls the store method |
...
137 rows, each carrying exactly one verdict. The 137 is a derivation and not a
carried figure, and it is re-run against the tables themselves at every pass
that touches them. At this head:

```
  135  data rows in the seven-column census tables
                (| id | defect | source | verdict | gate | evidence | justification |),
                counted by parsing the tables, not by reading them
+  13  DE-* data rows in the six-column deployability table
                (| id | item | source | verdict | evidence | justification |)
= 148  row ids, all distinct: no id appears in two tables and none appears twice
-  11  cross-references, which restate a row counted elsewhere
= 137  counted rows
```

The eleven are exactly these, and each names its target in the table itself:
PR-1 to PR-7 carry an explicit "(= AU-nn)" and restate AU-20, AU-29, AU-17,
AU-26, AU-12, AU-6 and AU-33; PR-10 restates AU-32; and TS-12, PR-11 and PR-12
are the same rows as DE-11, DE-9 and DE-8, which are the counted instances. The
DE side keeps the count because the DE row is where the item is established;
the PR and TS side restates it.
...
is the figure AT THE AUDIT; see the corrections below, which take it to 120,
then to 114, and then to **107 at this head** — where 107 was arrived at by
COUNTING the verdict column of all 137 counted rows, not by subtracting from
114. The two previous figures were arithmetic on a prior figure, which is how
a count survives the pass that should have invalidated it.

### The count after the remediation pass, derived rather than asserted

| | at the audit | after remediation | after the integration pass | after the census pass | **at this head, counted** |
|---|---|---|---|---|---|
| REACHABLE | 118 | **118** | **117** | **111** | **104** |
| GATED OFF | 3 | **3** | **3** | **3** | **3** |
| UNREACHABLE | 13 | **13** | **13** | **15** | **15** |
| CLOSED | 0 | **0** | **1** | **5** | **12** |
| UNDETERMINED | 3 | **3** | **3** | **3** | **3** |
| total | 137 | 137 | 137 | 137 | **137** |
| **blocking (REACHABLE + UNDETERMINED)** | **121** | **121** | **120** | **114** | **107** |

The integration-pass column is the correction that pass owes this table. OC-2 was
...
eleven cross-references named above leaves exactly 137 rows, of which 117 carried
a plain REACHABLE verdict AT THAT PASS, one of the two reading-dependent rows is
REACHABLE under either reading, and three are UNDETERMINED. 117 + 1 + 3 = 121 at
the audit, when OC-2 was still among the 117; 120 once it was not.

### The census column, derived the same way
...
Plain-REACHABLE therefore went 116 to 110, and 110 + 1 reading-dependent row + 3
UNDETERMINED = 114 at that pass. The verdict totals summed to 137: 111 + 3 + 15 +
5 + 3.
...
### The last column, COUNTED rather than derived

**114 was wrong at this head, and it was wrong in the way this section keeps
warning about: it was arithmetic on 120, which was arithmetic on 121.** Nobody
had counted the verdict column since the census pass, and two later passes
closed rows without touching this table. Counted at this head, by parsing the
verdict cell of all 137 counted rows:

  * **REACHABLE 103 plain**, plus the one reading-dependent row that is
    REACHABLE under whichever reading is taken (Reading A: GATED-1; Reading B:
    D-19a) = **104**.
  * **GATED OFF 2 plain** (JR-2, GATED-2), plus the other reading-dependent row
    = **3**.
  * **UNREACHABLE 15**: AU-24, AU-28 and DE-1 to DE-13.
  * **CLOSED 12**: OC-2, OV-4, OV-5, OV-7, OV-8 and SC-1 to SC-7.
  * **UNDETERMINED 3**: AL-13, SC-8, OC-3.

104 + 3 + 15 + 12 + 3 = 137, and **blocking = REACHABLE + UNDETERMINED = 104 + 3
= 107 of 137, under both readings.**

**The seven rows are SC-1 to SC-7**, and that is the whole of the difference
between 114 and 107. The Class 7 pass closed them — each reads "WAS REACHABLE
… CLOSED", three of them splitting the verdict across the sub-readers they
cover ("WAS REACHABLE for four and UNREACHABLE for two; CLOSED for both, with
different claims") — and did not move this table. That is the same omission the
integration pass made with OC-2, at seven times the size. It is recorded here
rather than quietly absorbed, because the failure mode is the count, not the
pass.

**This pass moved nine rows and not one of them is in that number, which is the
point of the third status.** OV-21, the merge half of OV-22 and the grant half
of AU-3 became REMEDIED, PENDING ACTIVATION; AU-9, AU-10, AU-11, AU-18, AU-21,
AU-32 and RY-1 became REMEDIED AS UNSUPPORTED, PENDING ACTIVATION. Every one of
the seven `ChainParams` fields behind them defaults to `None` and is set
nowhere in this branch, and `genesis.json` contains no `*_enabled_from_height`
key at all. **A gate that is not set is not GATED OFF**, so all nine keep the
REACHABLE verdict they had and the blocking count is 107 before this pass and
107 after it. What moved is the remedy column, and the count says so by not
moving.

**Otherwise unchanged, and the reason is the whole point.** A large majority of
the blocking rows now carry a
remedy that is implemented, reachable through a named seam and covered by tests
that show an ungated node and a gated node disagreeing, and several more carry
half of one. Every one of those
remedies sits behind an activation height that exists in `ChainParams` and is
set nowhere: `REMEDIATION_GATES` in `crates/genesis/src/lib.rs` names
**thirty-seven** of them at this head. An absent or `None`
`#[serde(default)] Option<u64>` closes the gate;
...
thirty-seven remediation fields now exist — `REMEDIATION_GATES` in
`crates/genesis/src/lib.rs` is the list, and it is pinned against the accessor
table the executors actually read — so the remaining half of the precondition
is that they are SET to a height in the deployed runtime `genesis.json`. When
they are, every row whose remedy is FULLY implemented moves from REACHABLE to
GATED OFF, the remediated behaviour becomes the behaviour, and the blocking
count falls.

**By how much is NOT stated here, and the omission is deliberate.** This
paragraph used to say "forty-eight rows, 114 to 66", and named them. That list
is stale: passes since then have added full remedies to rows outside it, this
pass added seven more, and the 114 it subtracted from was itself wrong (see the
counted column above — it is 107). Re-deriving the projection is not arithmetic
on a stale figure; it is a row-by-row judgement about which remedies are FULL
and which are half, and a row is GATED OFF only when the WHOLE of it is —
AU-34, AU-35, AU-37, RY-2, D-19b and the transfer-and-subdivide remainder of
OV-22 each carry only part. **This pass did not make that judgement and
therefore states no number**, because a projected blocking count carried
forward unchecked is the exact failure this document was written to prevent,
and quoting one would be committing it in the paragraph that warns against it.
What IS counted, and holds: **107 today**, and the rows that would move are
exactly those whose gate cell reads REMEDIED (or REMEDIED AS UNSUPPORTED)
PENDING ACTIVATION with no PARTLY and no named half.

Six rows were formerly filed **BLOCKED, STRUCTURAL** with no gate at all (AU-9,
AU-10, AU-11, AU-18, AU-21, AU-32), on the reasoning that no guard can close
them because the subsystem records no address, no registry and no signature to
authorize against. **That reasoning still holds and is not what changed. What
changed is the conclusion drawn from it:** "blocked" had been taken to mean
"leave it reachable", and the two are different things. An operation that
cannot be performed correctly must REFUSE; it may not proceed on a guess. All
six, and RY-1 beside them, now return a FAILED receipt naming the operation
UNSUPPORTED at their own activation height, ahead of the fee deduct, where each
subsystem's own refusals already return. The wire and registry changes in
`crates/sumchain-wire` and `ChainParams` remain the real fix and remain
unwritten, and all seven rows are REACHABLE and blocking until an operator sets
the heights.
...
The remediation passes change what is available, not what is shipped:
**thirty-seven** activation gates now exist in the executors, dormant, each
reading the `ChainParams` field it names, and `REMEDIATION_GATES` in
`crates/genesis/src/lib.rs` is the list. Until an operator sets them, the
...
### Tier 5 — correctness, data loss, and reachable-but-narrow
 ...
 26. **OC-3 (UNDETERMINED — EXTERNAL EVIDENCE).** Not settleable from this
     worktree at any effort. It carries an exact reproducer — the tree, the
     command and the output that closes it — in "Every UNDETERMINED, and
     what would settle it" below.

## Every UNDETERMINED, and what would settle it

Three rows. Each blocks, and each names its missing evidence.

**Two buckets, by where the missing evidence lives.** The distinction is
operational, not cosmetic: a row in the first bucket can be worked in this
worktree, and a row in the second cannot be, however many further passes are
run against it.

  * **EXTERNAL EVIDENCE — not settleable from this worktree, at any effort.**
    **OC-3**, whose evidence is a tree that is not here. Re-reading this
    repository returns the same answer every time and costs a pass each time,
    so the entry below carries an exact reproducer — the command, the tree to
    run it against, and the output that closes the row — rather than a
    description of the question. **SC-8** is the same shape (its evidence is
    the deployed operator configuration, which no amount of source reading
    produces); it is noted as such and otherwise left exactly where it is.
  * **INTERNAL — settleable here, by work not yet done.** **AL-13.** Its third
    input has arrived since the last pass; what is missing is now the
    arithmetic over three known numbers rather than one of the numbers.
  ...
  * **AL-13 — that arbitrary input never reaches an allocator abort.** The eight
    ...
    specification to compare against. The bincode measurement run for this
    audit closes the decode half (bounded at ~1 MiB per attempt) and not the
    row-growth half.

    **The third number has arrived, and it is why this row is in the INTERNAL
    bucket rather than the external one.** It used to be true that it "does not
    exist anywhere in this tree", and stronger than that: the tree
    affirmatively denied that a validator RAM minimum existed, so the
    comparison had no right-hand side that could honestly be supplied from
    source. The repository owner has since adopted **4 GiB as the supported
    validator memory floor for this release**, as an OPERATIONAL minimum and
    explicitly not a consensus-derived fact —
    `docs/operations/validator-memory-floor.md`,
    `docs/b0-pre/venue/VENUE.md`, and `requests.memory: "4Gi"` alongside
    `limits.memory: "4Gi"` in all four validator manifests under
    `deploy/kubernetes/`. It is NOT `reference_memory_bytes` re-labelled: that
    figure and its "candidate-comparison envelope, not a deployment or
    consensus hardware minimum" text are untouched, and the ratified
    `b0_pre_spec_hash` is unchanged. **The row stays UNDETERMINED.** What is
    missing is now the growth-to-abort arithmetic over three known numbers
    rather than one of the numbers, and that arithmetic is work this worktree
    can do.
  ...
  * **OC-3 — the rest of the designated out-of-consensus inventory.** The
    ...
    `crates/consensus/src/` contain no `MESSAGING` reference at all. I am not
    ...
    **BUCKET: EXTERNAL EVIDENCE.** The missing evidence is the tree, branch or
    commit the count of thirteen was taken against. It is not in this worktree,
    and no further reading of this worktree will produce it.

    **THE REPRODUCER.** Not "count the call sites" — the enumeration is already
    derived by the tree on every test run, by
    `crates/storage/tests/messaging_write_site_inventory.rs`, and the point of
    the procedure below is to run THAT derivation against the tree the thirteen
    was taken against. It is self-contained (`std` only; it reads
    `crates/storage/src/messaging_store.rs` and walks `crates/*/src`), so it
    transplants into another checkout without pulling anything else with it.

    1. Identify the tree `T` the designated count was taken against — the
       branch, commit or working copy the blocker document's "seven operator,
       two genesis, one snapshot and three raw reorg" was read off. **This step
       is the whole blocker. Steps 2-5 are mechanical once it is answered.**
    2. Record its identity, resolved and not abbreviated:

       ```
       git -C <T> rev-parse HEAD
       ```

    3. Copy this worktree's derivation into `T`, unmodified:

       ```
       cp crates/storage/tests/messaging_write_site_inventory.rs \
          <T>/crates/storage/tests/messaging_write_site_inventory.rs
       ```

    4. Run both of its tests in `T`, with the discriminator, and capture the
       output:

       ```
       cd <T> && cargo test -p sumchain-storage \
         --test messaging_write_site_inventory -- --nocapture
       ```

    5. Read the result. There are exactly three outcomes and each one closes or
       redirects the row:

       * **`the_scan_can_see_a_caller_it_is_not_expecting` FAILS.** The scan is
         blind in `T` — the walk missed `crates/node`, or the writer-method
         derivation came back empty, or `T`'s layout is not `crates/*/src`. The
         other result is vacuous and must be discarded. Fix the walk for `T`'s
         layout and re-run; do not read the other test's outcome until this one
         passes.
       * **Both tests PASS.** `T` contains exactly the same two non-test callers
         this worktree does — `backfill_indexes` (OC-1) and
         `seed_registry_at_genesis` (OC-2). The designated thirteen is then not
         a count of out-of-consensus messaging write CALL SITES against `T`
         either, and the designation itself is what needs correcting. **This
         closes OC-3 as a designation error**, with `T`'s SHA from step 2 as the
         evidence.
       * **`the_non_test_callers_of_a_messaging_write_are_exactly_the_two_audited_rows`
         FAILS.** Its `assert_eq!` prints `left` (found in `T`) against `right`
         (the two audited rows) as a `BTreeMap<file path, [method names]>`.
         **That printed `left` map IS the enumeration this row has been asking
         for**, with a file path and a method name for every site. If it has
         thirteen entries, the designated count is reproduced and each entry
         becomes an audit row of its own, classified like OC-1 and OC-2. If it
         has some other number, the count is wrong against `T` as well, and the
         reproduced number with `T`'s SHA is what replaces it.

    **What output closes the row, stated as one line:** the `left` map from step
    5 (or a clean pass), together with `git -C <T> rev-parse HEAD` from step 2.
    Nothing less closes it, because without the SHA the enumeration cannot be
    attributed to a tree, and an enumeration attributable to no tree is how the
    count of thirteen came to be unreconcilable in the first place.

    **What does NOT close it:** any recount performed inside this worktree;
    counting `put`/`delete` STATEMENTS inside `messaging_store.rs` or
    `messaging_view.rs` as call sites; or reclassifying until two and thirteen
    agree.
...
| row(s) | why |
|---|---|
| AU-9, AU-10, AU-11 | `AgreementCommitment` carries no address at all, and `PartyRef` is a 32-byte commitment or a 32-byte subject id. There is nothing in the subsystem to compare a sender to, so no guard can be written. Needs an `Address` on the commitment (or an address-bearing `PartyRef` variant) in `crates/sumchain-wire/src/agreement.rs`; AU-10 additionally needs a canonical signing input the subsystem does not define. **SINCE: still true, and no longer the end of it.** A later pass took "no guard can be written" to mean the operation must REFUSE rather than stay reachable, and the row now carries `agreement_party_authority_unsupported_enabled_from_height`. The wire change is still the fix and is still unwritten |
| AU-3 (grant half) | a consent's subject cannot participate in GRANTING without a signature `ConsentEnvelope` does not carry. The revocation half is remediated; the grant half is a wire change. **SINCE: the wire change was WRITTEN, so this entry is history and AU-3 is no longer partial.** The diagnosis was also half wrong: `ConsentEnvelope` does carry `subject_address`, so what was missing was the SIGNATURE alone. `ConsentGrantRequest` supplies it, behind `healthcare_consent_subject_signature_enabled_from_height` |
| AU-18, AU-21 | the tax and finance registries authorize nothing they do not take from the applicant, and no `ChainParams` field names a registrar for either. Inventing one inside an executor would be a rule nobody set. **SINCE: still true, and no longer the end of it.** A later pass took "no guard can be written" to mean the operation must REFUSE rather than stay reachable, and the row now carries `subsystem_issuer_self_registration_unsupported_enabled_from_height`, one field for both. The registrar is still the fix and is still absent |
| AU-32 | `PropertyProofEnvelope` carries no issuer address and Property has no issuer registry at all. **SINCE: still true, and no longer the end of it.** A later pass took "no guard can be written" to mean the operation must REFUSE rather than stay reachable, and the row now carries `property_proof_submission_unsupported_enabled_from_height`. Nothing is taken away by refusing: the only reader of a row `SubmitProof` writes is the `VerifyProof` arm, which already refuses as UNSUPPORTED |
...
| OV-21, OV-22 (merge half), RY-1 | **SINCE CLOSED, and the entries above that call them "untouched" are history rather than current state.** The two `OV-21..OV-24` rows in this table and the `Class 6 (RY-1..RY-3)` rows were written by passes that did not reach them; later passes did. OV-21 is remedied behind `property_state_precondition_enabled_from_height` (twenty-two guard sites, all ahead of the fee), OV-22's merge half behind `property_asset_relationship_enabled_from_height` (both rows linked, list bounded), and RY-1 refuses an unpayable royalty at creation behind `nft_unpayable_royalty_refused_enabled_from_height`. OV-22's transfer and subdivide halves and OV-11 are the ones still genuinely BLOCKED, STRUCTURAL, and they are the two rows above this one. Recorded as a correction rather than by deleting the stale entries, because what a pass did NOT reach is part of this table's value |
