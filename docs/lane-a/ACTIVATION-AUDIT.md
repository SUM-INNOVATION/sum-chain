...
| AU-24 | `UpdateIssuer`'s `issuer.issuer_address != *sender` check is a no-op | Finance 4 | **REACHABLE** | none. **NOTED -- a dead check, not a hole: the row is fetched BY the sender key and registration forces the equality the comparison later tests, so the comparison cannot fire and removing it would close nothing** | row fetched by sender key at `crates/state/src/finance_executor.rs:177`, compared at `:182`; rows keyed by `issuer_address` at `crates/state/src/finance_view.rs:98`; registration forces equality at `:152` | the comparison is structurally unable to fire -- now ASSERTED rather than read off the source. `the_issuer_sender_check_fires_only_on_a_row_no_transaction_can_write` (`sumchain-state --test revocation_index_meaning`) shows both halves: the comparison DOES fire against a hand-planted row whose key and `issuer_address` differ, so it is live code and not something a compiler removed; and after registration and after every status-changing operation the row reachable at the sender's key always names the sender, because the only writer of the family keys by the value's own address and registration refuses anything else. Still nothing to close |
...
| OV-4 | Employment `update_status` and `revoke` rewrite only the credential row; the three indexes built at creation keep pointing at a credential that is now `Ended` | Employment 2 | **REACHABLE, AND THE ROW IS WITHDRAWN AS A DEFECT.** The code runs, the fact it states is exactly right, and the fact is the subsystem working rather than failing. It stays in the blocking count's denominator the way OV-16 does: a withdrawn consequence is a finding, not a closure | none, and none is wanted | pinned by `revoking_a_credential_leaves_all_three_index_entries_behind` and `an_update_and_a_revoke_rewrite_only_the_credential_row`; the WITHDRAWAL is pinned by `the_employee_index_is_what_the_ended_count_is_computed_from`, `the_status_filter_is_at_the_reader_and_the_unfiltered_reader_is_public` and `an_ended_credential_can_be_restored_so_removing_its_index_entry_would_be_irreversible` (`sumchain-state --test revocation_index_meaning`) | the second test is in the codec-parity suite, which asserts storage bytes rather than driving dispatch — it proves the row shape, not the route. **The behaviour is correct and the row's implied remedy is the defect.** `EmploymentCredentialStore::summarize_by_employee` computes `employment_getSummary`'s `ended` count by walking the employee index, so an index that dropped the entry would report `ended = 0` for every employee who has ever left a job; every reader is PAIRED (`get_by_employee` beside `get_active_by_employee`, and both reach the RPC as separate methods), which is only coherent over an index that holds everything; and `UpdateEmployment` accepts any status including `Active`, while no path on the execution side ever re-adds an index entry, so a removal would be irreversible |
| OV-5 | Employment income attestation revocation touches neither income index | Employment 3 | **REACHABLE, AND THE ROW IS WITHDRAWN AS A DEFECT,** on OV-4's evidence and by OV-4's spelling | none, and none is wanted | pinned by `a_revoked_attestation_keeps_its_key_and_its_two_index_rows`, also a codec-parity test; the WITHDRAWAL is pinned by `the_two_income_indexes_are_read_by_the_same_all_and_valid_pair` (`sumchain-state --test revocation_index_meaning`) | same caveat as OV-4, and withdrawn on OV-4's evidence: `get_by_subject`/`get_valid_by_subject` and `get_by_holder_address`/`get_valid_by_holder_address` are the same all-and-valid pairs, so the two income indexes are the "all" half by the subsystem's own construction |
...
| OV-7..OV-9, OV-11, OV-15, OV-16, OV-19, OV-21..OV-24, OV-27, CI-1, CO-1, MD-1, MD-2, PA-1 | untouched by the Class 8 pass that closed the ten rows above, and still untouched. (OV-6, OV-10, OV-25 and OV-30 have since been closed and have left this list; OV-11 and the transfer half of OV-22 are BLOCKED, STRUCTURAL in the row below.) The pass chose depth over breadth: four gates, ten rows, each with a mixed-version test that shows an ungated and a gated node disagreeing and a closed-gate test that reproduces today's behaviour exactly. Nothing about these rows changed, and none of them was investigated far enough to move a verdict. **A later NFT-and-Agreement pass has since moved five of them and left this list: OV-9, OV-15 and CI-1 are REMEDIED, PENDING ACTIVATION behind three new heights; OV-16's stated CONSEQUENCE was tested against source and withdrawn -- the mechanism it names does not hold, and the opposite one does; and OV-11 was re-tested rather than inherited and stays BLOCKED, STRUCTURAL with a second leg. AL-5 moved in the same pass, on the shared allocation-bound height. An Employment-and-Finance pass has since moved two more, both by OV-16's route rather than by a gate: OV-7's and OV-8's stated CONSEQUENCES were tested against source and WITHDRAWN, and the rows say what replaced them. AL-2 and AL-4 moved in that pass on the shared allocation-bound height, which now has five readers. OV-19, OV-21..OV-24, OV-27, CO-1, MD-1, MD-2 and PA-1 are still untouched** |
