...
| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| PR-1 | Tax `VerifyProof` (= AU-20) | Tax 6 | **REACHABLE** | none | `crates/state/src/tax_executor.rs:275`, body `:277-281` | no proof read of any kind |
| PR-2 | Employment `VerifyProof` (= AU-29) | Employment 6 | **REACHABLE** | none | `crates/state/src/employment_executor.rs:506`, body `:508-512` | does not even check the proof exists |
| PR-3 | Legal `VerifyProof` (= AU-17) | Legal 5 | **REACHABLE** | none | `crates/state/src/legal_executor.rs:768`, body `:770-774` | succeeds for a payload that is not a proof id |
| PR-4 | Finance `VerifyProof` (= AU-26) | Finance 7 | **REACHABLE** | none | `crates/state/src/finance_executor.rs:528`, body `:530-534` | succeeds for a proof id that does not exist |
| PR-5 | Agreement `VerifyProof` (= AU-12) | Agreement | **REACHABLE** | none | `crates/state/src/agreement_executor.rs:621`, body `:623-627` | does not deserialize the payload |
| PR-6 | Healthcare `VerifyProof` (= AU-6) | Healthcare | **REACHABLE** | none | `crates/state/src/healthcare_executor.rs:955`, body `:957-961` | — |
...
| id | defect | source | verdict | gate | evidence | justification |
|---|---|---|---|---|---|---|
| SC-1 | Tax RPC list methods scan whole column families into `Vec`s with no pagination | Tax §unbounded reads | **REACHABLE** | none | `tax_listClaimTypes` `crates/rpc/src/api.rs:1193` → `crates/rpc/src/server.rs:4326` → `crates/storage/src/tax_store.rs:147`; `tax_getActiveIssuers` `api.rs:1206` → `server.rs:4344` → `tax_store.rs:263`; `tax_listPolicies` `api.rs:1226` → `server.rs:4373` → `tax_store.rs:312` | three public methods, three full-family scans, no limit parameter on any |
| SC-2 | `employment_list_issuers` scans the whole issuer family; seven `get_by_*` readers decode an entire index vector and then collect every referenced record into a second vector, with the `active`/`valid` variants building the full list before filtering | Employment 11 | **REACHABLE** | none | `employment_listIssuers` `crates/rpc/src/api.rs:1625` → `crates/rpc/src/server.rs:5769` → `crates/storage/src/employment_store.rs:226` (full scan); index-driven readers at `api.rs:1638, 1645, 1652, 1681, 1692, 1699, 1706` → `server.rs:5822, 5848, 5874, 6004, 6027, 6046, 6065`; also reached from `employment_verifyEmployment` `api.rs:1659` → `server.rs:5905` | every one of the eight the source message names is publicly reachable; none takes a limit, offset or cursor |
| SC-3 | `FinanceStore::issuers().list_active()` and `get_by_jurisdiction()` are unpaginated and unbounded | Finance 13 | **REACHABLE** | none | `finance_getActiveIssuers` `crates/rpc/src/api.rs:1363` → `crates/rpc/src/server.rs:4543` → `crates/storage/src/finance_store.rs:225` (full scan); `finance_getIssuersByJurisdiction` `api.rs:1369` → `server.rs:4553` → `finance_store.rs:237` | the RPC constructs `FinanceIssuerStore` directly rather than through the `FinanceStore::issuers()` facade (`finance_store.rs:774`), which is unused — same store, same scan |
| SC-4 | Agreement `list_active` and `get_by_agreement` walk every row in their column family and return one `Vec` | Agreement §unbounded reads | **REACHABLE** | none | `agreement_getActiveExecutorLinks` `crates/rpc/src/api.rs:1318` → `crates/rpc/src/server.rs:4492` → `crates/storage/src/agreement_store.rs:663`; `agreement_getExecutorLinksByAgreement` `api.rs:1304` → `server.rs:4474` → `agreement_store.rs:639`; pinned by `the_committed_agreement_readers_return_two_thousand_rows_whole` | `get_by_agreement` is a **full scan plus filter**, not an index read — worse than the source message's description |
| SC-5 | Property `list_active` walks every row; the four `get_by_*` readers resolve an index list and point-read every id | Property §unbounded reads | **REACHABLE** for two, **UNREACHABLE** for the rest | none | `property_getActiveAssets` `crates/rpc/src/api.rs:1337` → `crates/rpc/src/server.rs:4512` → `crates/storage/src/property_store.rs:281` (full scan); `property_getAssetsByJurisdiction` `api.rs:1343` → `server.rs:4522` → `property_store.rs:269`; the remaining property `get_by_*` readers have **no** RPC method | split verdict stated explicitly rather than averaged; the pinning test `the_committed_property_readers_return_two_thousand_rows_whole` exercises readers that production cannot call |
| SC-6 | Healthcare `list_active` walks every row; `get_by_network`, `get_by_member`, `get_by_subject`, `get_by_patient`, `get_by_prescriber` each resolve a whole index list row by row | Healthcare §unbounded reads | **REACHABLE** for `list_active`, **UNREACHABLE** for the five `get_by_*` | none | `healthcare_getActiveInstitutionalProviders` `crates/rpc/src/api.rs:1419` → `crates/rpc/src/server.rs:4634` → `crates/storage/src/healthcare_store.rs:317` (full scan, filtered to institutional providers after the scan); no RPC method reaches `healthcare_store.rs:305, 517, 633, 820, 842` | the five patient- and subject-facing readers are the sensitive ones and none is exposed — worth recording precisely, because it is the one place the RPC surface is narrower than the storage surface |
| SC-7 | DocClass `get_all`, `get_active`, `get_by_jurisdiction`, `get_by_controller`, `get_by_subcode`, `get_by_revoker` each walk a whole family and return one `Vec` | DocClass §unbounded reads | **REACHABLE** for four, **UNREACHABLE** for two | none | `docclass_getIssuers` `crates/rpc/src/api.rs:1166` and `docclass_getSummary` `api.rs:1107` → `crates/rpc/src/server.rs:4283, 4029` → `crates/storage/src/docclass_store.rs:757`; `docclass_getIssuersByJurisdiction` `api.rs:1174` → `server.rs:4302` → `docclass_store.rs:774` (which itself calls `get_all` then filters); `docclass_getIdentityByController` `api.rs:1120` → `server.rs:4072` → `docclass_store.rs:232` (full scan); `docclass_getAcademicCredentialsByHolder` `api.rs:1772` → `server.rs:7412` → `docclass_store.rs:485`, **looped over three subcodes, so three full family scans per call**; `get_active` (`docclass_store.rs:768`), `get_by_subcode` on issuers (`:783`) and `get_by_revoker` (`:655`) have no RPC method | `docclass_getIssuers` is the only method in the whole audit that takes `limit`/`offset` (`api.rs:1166-1171`) and it still scans the entire family first, applying `.skip()/.take()` in memory at `server.rs:4283-4293` |
| SC-8 | The claim that the release RPC surface is safe to expose publicly at all | not asserted in the blocker document; raised by this audit | **UNDETERMINED** | — | auth built (`crates/rpc/src/auth.rs`, `ApiKeyValidator::authorize`) and never called on the request path — `crates/rpc/src/server.rs:181, 296, 399`; rate limiting likewise `:182, 404`; default bind `127.0.0.1:8545` (`crates/node/src/config.rs:201`) | **what would settle it:** the deployed operator configuration — whether production validators bind loopback with a reverse proxy in front, or set `addr = "0.0.0.0:8545"` as `crates/node/src/config.rs:380-385` shows is supported. The tree cannot answer it. Under loopback the fourteen scans above are operator-facing; under a public bind they are an unauthenticated, unrate-limited denial surface |
...
| OV-6 | Legal repeated `ConsolidateCase` is a paid no-op: the append is skipped when the relation is recorded, and `updated_at` is written only inside that branch | Legal 7 | **REACHABLE** | none | `v_add_related_case` is `contains`-gated at `crates/state/src/legal_view.rs:145`; the arm still charges at `crates/state/src/legal_executor.rs:289-291`; pinned by `a_repeated_consolidation_is_a_paid_no_op` | — |
...
| OV-12 | NFT creator may rewrite the metadata of a token it no longer owns, for the life of the token | NFT §authority | **REACHABLE** | none | guard `crates/state/src/nft_executor.rs:645` is `token.owner != *sender && token.creator != *sender`; wholesale overwrite `:652`; pinned by `the_creator_can_rewrite_metadata_of_a_token_it_no_longer_owns` | `creator` never changes |
| OV-13 | NFT `locked` flag is consulted by transfer and burn only; a locked token can still be approved and have its metadata rewritten | NFT §authority | **REACHABLE** | none | consulted at `crates/state/src/nft_executor.rs:515` and `:606` only; not at `:545` (approve), `:623` (update metadata) or `:665` (collection transfer); pinned by `a_locked_token_can_still_be_approved_and_rewritten` | — |
| OV-14 | NFT `Approve` never reads the collection, so an approval can be recorded on a token in a non-transferable collection | NFT §authority | **REACHABLE** | none | `execute_approve` `crates/state/src/nft_executor.rs:545-577` makes no `v_get_collection` call; only guard is `token.owner != *sender` at `:557`; pinned by `approve_never_reads_the_collection` | — |
...
| OV-30 | Agreement `AddParty` and `RemoveParty` charge a fee, advance the nonce and do nothing | Agreement §overwrite | **REACHABLE** | none | arm `crates/state/src/agreement_executor.rs:322`, body `:324-328`; pinned by `add_party_and_remove_party_charge_a_fee_and_do_nothing` | — |
