...
| Reward / slash / dispute tx families | Settlement and dispute are a separate protocol surface, referenced by `(session_id, verifier_address)`. v1 commits to attestation immutability; economics come later. **Now specified + implemented as the separate [Inference Settlement](inference-settlement.md) subprotocol, active on mainnet (gate set to 8,900,000, issue #61)** — escrow-funded reward denial / claim withholding / escrow refund; **no bond slashing** in the base settlement (a verifier-bond registry ships as the separate `inference_verifier_bonding` gate at height 9,200,000). Attestation v1 is untouched. | [inference-settlement.md](inference-settlement.md) (issue #61) |
...
- **Storage & dedup unchanged.** The record is stored under `inference_attestation_key(session_id, verifier_address)` — the same keyspace as v1 — so v1 and v2 cannot double-submit the same `(session_id, verifier)` (second attempt → `Failed(51)`). The stored `InferenceAttestationRecord` is **identical to v1 and verifier-attributed**. The sponsor is **not** part of the record; issue #95 records it as **additive side metadata** in a dedicated CF (see below), never in `InferenceAttestationRecord`.
- **Sponsor metadata (additive, issue #95).** On a sponsored v2 submission the chain also writes an `InferenceAttestationSponsor { sponsor, submitted_at_height, tx_hash }` to the `inference_attestation_sponsors` CF, keyed by the **same** `inference_attestation_key(session_id, verifier_address)`, in the **same atomic batch** as the record (both commit or neither). v1 direct submissions write no sponsor entry. This is observability-only — it does not change the record, the wire, the dedup key, or settlement (which still pays the verifier). Read it with `sum_getInferenceAttestationSponsor(session_id, verifier_address)`, which returns `null` for v1/absent attestations. There is intentionally **no** sponsor-wide reverse lookup (privacy).
...
- **Gate.** `omninode_sponsored_attestation_enabled_from_height: Option<u64>` (fresh-chain default `None`). On mainnet this gate is **deployed in runtime genesis and activation-gated at height 9,200,000** — one of the seven post-supply gates, and separate from base attestation, which is already active. Sponsored v2 submission is therefore **not usable before height 9,200,000** and activates automatically when the chain reaches it. While closed, a v2 tx is rejected free (`Failed(54)`, no fee/mutation), symmetric with the `Failed(50)` v1 gate; v1 submission is unaffected (governed only by `omninode_enabled_from_height`). This gate is not exposed by `chain_getChainParams`, so runtime genesis is the source of truth. A malformed envelope (invalid verifier public key, or oversize `session_id`) → `Failed(55)`.
